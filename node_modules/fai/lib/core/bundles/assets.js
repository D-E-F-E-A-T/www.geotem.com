var Axis, CSSo, Coffee, CoffeeProcess, Express, FS, Files, Jeet, Nib, Path, Prefixr, Regex, Route, Rupture, Stylus, TmplJS, Types, Uglify, Zlib, tmpdir;
...
CoffeeProcess = function(orig, pth) {
  var e, ext, file, index, match, path, regex, str;
...
Types = {
  css: {
    ...
    run: function(str, path) {
      return Stylus(str).set('paths', [ﬁ.path.app.master, path]).set('include css', true).use(Rupture()).use(Axis()).use(Jeet())["import"]('jeet').use(Nib()).use(Prefixr()).render();
    },
    min: function(str) {
...
module.exports = {
  ...
  locals: function() {
    ...
    uri = function(type, name) {
      return Path.join(Route, type, name + '.' + type);
    };
    return {
      css: function(name) {
        return uri('css', name);
